...
** Flashgg setup for the very low mass analysis in CMSSW_10_6_8:
   ...
   git remote -v 
   git fetch myFlashGG 
   git merge myFlashGG/EF_LMfromAnna
   cd ../
   scram b -j 8
   #+END_EXAMPLE
 ...
 * Manual test for microAOD production with a single file:
   #+BEGIN_EXAMPLE
   cmsRun MicroAOD/test/microAODstd.py processType=[sig] datasetName=glugluh conditionsJSON=MetaData/data/MetaConditions/Era2018_Legacy_lowMassDiphotonAnalysis.json
   #+END_EXAMPLE
   
 * MicroAOD step on a dataset (note that you need a proper json file in the =campaign= directory):
   #+BEGIN_EXAMPLE
   ./prepareCrabJobs.py -V v0 -C analysisLM_Leg18 --meta-conditions /afs/cern.ch/work/e/elfontan/private/DiPhotonAnalysis/Anna_Legacy2018/CMSSW_10_6_8/src/flashgg/MetaData/data/MetaConditions/Era2018_Legacy_lowMassDiphotonAnalysis.json  -O T2_US_MIT -o /store/user/elfontan -s campaigns/TEST_M70_Legacy.json --mkPilot
   #+END_EXAMPLE

   The output is located in DAS in the =prod/phys03= database in the form:
   #+BEGIN_EXAMPLE
   /GluGluHToGG_M70_TuneCP5_13TeV-amcatnloFXFX-pythia8/elfontan-analysisLM_Leg18-v0-v0-RunIIAutumn18MiniAOD-102X_upgrade2018_realistic_v15-v1-b843de9981bbcd085fdb69f725c69efd/USER
   #+END_EXAMPLE
 ...
 * Flashgg step:
   ...
   #+BEGIN_EXAMPLE
   fggManageSamples.py -C analysisLM_Leg18 import '/GluGluHToGG_M*/elfontan-analysisLM_Leg18*/USER'
   #+END_EXAMPLE
   The output will be located in a subdirectory of =MetaData/data= with the same name of the campaign, e.g.:
   #+BEGIN_EXAMPLE
   /afs/cern.ch/work/e/elfontan/private/DiPhotonAnalysis/Anna_Legacy2018/CMSSW_10_6_8/src/flashgg/MetaData/data/analysisLM_Leg18/datasets.json  
   #+END_EXAMPLE   
   ...
   *the =X509_USER_PROXY= environment variable and run with the =--no-copy-proxy= option*
   ...
   fggRunJobs.py --load analysisLM_Leg18.json -d outdir_analysisLM_Leg18 workspaceStd.py -n 300 -q workday --no-copy-proxy --no-use-tarball
   #+END_EXAMPLE 
