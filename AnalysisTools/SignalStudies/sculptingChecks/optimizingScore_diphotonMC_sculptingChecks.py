...
# Data from the sideband
f_dipho = TFile("/eos/user/e/elfontan/DiPhotonAnalysis/diphotonBDT/NTUPLES_May2024/"+massHyp+"/out_dipho_bkg_newSamplesFlat.root","READ") 
...
for ev_dipho in t_dipho:
    ...
    if (ev_dipho.NNScore >= 0.6 and ev_dipho.NNScore < 0.8):
        h_diphoMass_pre1.Fill(ev_dipho.dipho_mass, ev_dipho.weight*ev_dipho.weight_allDD)    
    if (ev_dipho.NNScore >= 0.5 and ev_dipho.NNScore < 0.6):
...
c_score.Update()
c_score.SaveAs(outputdir+"/nnScore_MCdiphoton.png")
c_score.SaveAs(outputdir+"/nnScore_MCdiphoton.pdf")
...
leg.AddEntry(h_diphoMass_pre0,"Diphoton MC: NN score > 0.8")
leg.AddEntry(h_diphoMass_pre1,"Diphoton MC: NN score = [0.6,0.8]")
leg.AddEntry(h_diphoMass_pre2,"Diphoton MC: NN score = [0.5,0.6]")
leg.SetLineWidth(0)
leg.Draw("same")
...
c1.Update()
if (normFactor):
    c1.SaveAs(outputdir+"/diphoMass_MCdiphoton_nnScoreOpt_norm1.png")
    c1.SaveAs(outputdir+"/diphoMass_MCdiphoton_nnScoreOpt_norm1.pdf")
else:
    c1.SaveAs(outputdir+"/diphoMass_MCdiphoton_nnScoreOpt.png")
    c1.SaveAs(outputdir+"/diphoMass_MCdiphoton_nnScoreOpt.pdf")
