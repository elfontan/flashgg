...
# python3  0_newR_dataMC_allVars_dd.py --log True  --outdir /eos/user/e/elfontan/www/Hgg_veryLowMass_AN/inputPreparation/0_TFracFitter/SR_m80
...
var_list = ["dipho_mass", "dipho_lead_ptoM", "dipho_sublead_ptoM", "dipho_leadEta", "dipho_subleadEta", "cosphi", "dipho_leadIDMVA", "dipho_subleadIDMVA", "sigmaMrvoM", "sigmaMwvoM", "vtxprob", "dipho_lead_sigmaEoE", "dipho_sublead_sigmaEoE", "dipho_pt", "dipho_sumpt" ]
label_list = ["m_{#gamma#gamma}", "Leading photon p_{T}", "Subleading photon p_{T}", "Leading photon #eta", "Sublead photon #eta", "cos#phi", "Lead IDMVA", "Sublead IDMVA", "#sigma_{RV}/m_{#gamma#gamma}", "sigma_{WV}/m_{#gamma#gamma}", "Vtx probability", "Leading photon #sigma_{E}/E", "Subleading photon #sigma_{E}/E", "Diphoton p_{T}", "Diphoton sum p_{T}" ]
...
for variable in var_list:
    ...
    if (cr_phoId0p0):
        sb0.Draw(variable + ">>h_" + variable + "_sbb0", "(CMS_hgg_mass>=10 && CMS_hgg_mass<75 &&  min(dipho_leadIDMVA>0.9,dipho_subleadIDMVA)>0.9)", "goff")
        sb0.Draw(variable + ">>h_" + variable + "_sba0", "weight*weight_sigmaEOE*weight_ptOverM*weight_eta* (CMS_hgg_mass>=10 && CMS_hgg_mass<75 && min(dipho_leadIDMVA>0.9,dipho_subleadIDMVA)>0.9)", "goff")
        dat0.Draw(variable + ">>h_" + variable + "_pre0", "CMS_hgg_mass>=10 && CMS_hgg_mass<75 && min(dipho_leadIDMVA,dipho_subleadIDMVA)>0.9 && event%20==0", "goff")
        mgg0.Draw(variable + ">>h_" + variable + "_mgg0", "weight * (CMS_hgg_mass>=10 && CMS_hgg_mass<75 && min(dipho_leadIDMVA,dipho_subleadIDMVA)>0.9)", "goff")
    else:
        sb0.Draw(variable + ">>h_" + variable + "_sbb0", "(CMS_hgg_mass>=10 && CMS_hgg_mass<75)", "goff")
        sb0.Draw(variable + ">>h_" + variable + "_sba0", "weight*weight_sigmaEOE*weight_ptOverM*weight_eta * (CMS_hgg_mass>=10 && CMS_hgg_mass<75)", "goff")
        dat0.Draw(variable + ">>h_" + variable + "_pre0", "CMS_hgg_mass>=10 && CMS_hgg_mass<75 && min(dipho_leadIDMVA,dipho_subleadIDMVA)>-0.7 && event%20==0", "goff")
        mgg0.Draw(variable + ">>h_" + variable + "_mgg0", "weight * (CMS_hgg_mass>=10 && CMS_hgg_mass<75 && min(dipho_leadIDMVA,dipho_subleadIDMVA)>-0.7)", "goff")
    #print(">>>>>>>>>>>>>>>>>>>>>>>>>>> FILLED HISTOS")
