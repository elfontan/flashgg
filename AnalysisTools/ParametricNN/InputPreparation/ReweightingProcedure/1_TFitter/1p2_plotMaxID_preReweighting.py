...
# python3  0p2_plotMaxID_preRewighting.py --log True  --outdir /eos/user/e/elfontan/www/Hgg_veryLowMass_AN/inputPreparation/0_TFracFitter/
...
# -----------------------------------------------------------------------------------------------------
#sb0.Draw("min(dipho_leadIDMVA,dipho_subleadIDMVA)>>sbb0","CMS_hgg_mass<80 && event%20==0 && min(dipho_leadIDMVA,dipho_subleadIDMVA)>=0.9","goff")
#sb0.Draw("min(dipho_leadIDMVA,dipho_subleadIDMVA)>>sba0","weight*weight_leadSublead_all * (CMS_hgg_mass<80 && event%20==0 && min(dipho_leadIDMVA,dipho_subleadIDMVA)>=0.9)","goff")
#dat0.Draw("min(dipho_leadIDMVA,dipho_subleadIDMVA)>>pre0","CMS_hgg_mass<80 && min(dipho_leadIDMVA,dipho_subleadIDMVA)>=0.9 && event%20==0","goff")
#mgg080.Draw("min(dipho_leadIDMVA,dipho_subleadIDMVA)>>mgg0","weight*(CMS_hgg_mass<80 && min(dipho_leadIDMVA,dipho_subleadIDMVA)>=0.9)","goff")
sb0.Draw("max(dipho_leadIDMVA,dipho_subleadIDMVA)>>sbb0","CMS_hgg_mass<80","goff")
sb0.Draw("max(dipho_leadIDMVA,dipho_subleadIDMVA)>>sba0","weight*(CMS_hgg_mass<80)","goff")
dat0.Draw("max(dipho_leadIDMVA,dipho_subleadIDMVA)>>pre0","CMS_hgg_mass<80 && min(dipho_leadIDMVA,dipho_subleadIDMVA)>=-0.7 && event%20==0","goff")
mgg080.Draw("max(dipho_leadIDMVA,dipho_subleadIDMVA)>>mgg0","weight*(CMS_hgg_mass<80 && min(dipho_leadIDMVA,dipho_subleadIDMVA)>=-0.7)","goff")
...
sbb0.SetLineColor(kBlue-10)
sbb0.SetFillColorAlpha(kBlue-10,0.8)
sbb0.SetLineWidth(2)

sbb0.GetYaxis().SetTitle("Events/0.05")
sbb0.GetYaxis().SetTitleSize(25)
sbb0.GetYaxis().SetTitleFont(43)
sbb0.GetYaxis().SetTitleOffset(0.9)
...
leg = TLegend(0.25,0.6,0.75,0.88)
...
rp.SetMaximum(1.8)
rp.SetStats(0)
rp.Divide(sbb0+mgg0) #divide by sideband+mgg                                                                                                     
rp.SetMarkerStyle(20)
rp.SetTitle("")

rp.SetYTitle("Presel / (Unweighted SB + #gamma#gamma MC)")
...
c1.Update()
if (logScale):
    c1.SaveAs(outputdir+"/maxPhoID_preReweighting_log.png")
    c1.SaveAs(outputdir+"/maxPhoID_preReweighting_log.pdf")
else:
    c1.SaveAs(outputdir+"/maxPhoID_preReweighting.png")
    c1.SaveAs(outputdir+"/maxPhoID_preReweighting.pdf")
