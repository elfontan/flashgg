...
data = TH1F("data", "Minimum #gamma MVA ID in data", 34, -0.7, 1)
#data = TH1F("data", "Minimum #gamma MVA ID in data", 85, -0.7, 1)
...
h_phoId_sba0 = TH1F("h_phoId_sba0", "Minimum #gamma MVA ID in data SB", 34, -0.7, 1.)
h_phoId_mgg0 = TH1F("h_phoId_mgg0", "Minimum #gamma MVA ID in diphoton MC", 34, -0.7, 1.)
h_phoId_mc = TH1F("h_phoId_mc", "Minimum #gamma MVA ID in PF+FF dd & #gamma#gamma", 34, -0.7, 1.)
h_phoId_mcScaled = TH1F("h_phoId_mcScaled", "Minimum #gamma MVA ID in PF+FF dd & #gamma#gamma", 34, -0.7, 1.)
#h_phoId_sba0 = TH1F("h_phoId_sba0", "Minimum #gamma MVA ID in data SB", 85, -0.7, 1.)
#h_phoId_mgg0 = TH1F("h_phoId_mgg0", "Minimum #gamma MVA ID in diphoton MC", 85, -0.7, 1.)
#h_phoId_mc = TH1F("h_phoId_mc", "Minimum #gamma MVA ID in PF+FF dd & #gamma#gamma", 85, -0.7, 1.)

# Fill the histograms with data and MC samples
# --------------------------------------------
#sb0.Draw("min(dipho_leadIDMVA,dipho_subleadIDMVA)>>h_phoId_sba0", "weight*(CMS_hgg_mass>0 && event%20==0 && min(dipho_leadIDMVA,dipho_subleadIDMVA)>-0.7)","goff")
#sb0.Draw("min(dipho_leadIDMVA,dipho_subleadIDMVA)>>h_phoId_sba0", "weight*(CMS_hgg_mass>0 && min(dipho_leadIDMVA,dipho_subleadIDMVA)>-0.7)","goff")
sb0.Draw("min(dipho_leadIDMVA,dipho_subleadIDMVA)>>h_phoId_sba0", "weight*(CMS_hgg_mass>0)","goff")
dat0.Draw("min(dipho_leadIDMVA,dipho_subleadIDMVA)>>data", "CMS_hgg_mass>0 && event%20==0 && min(dipho_leadIDMVA,dipho_subleadIDMVA)>-0.7","goff")
mgg0.Draw("min(dipho_leadIDMVA,dipho_subleadIDMVA)>>h_phoId_mgg0", "weight*(CMS_hgg_mass>0 && min(dipho_leadIDMVA,dipho_subleadIDMVA)>-0.7)","goff")
...
if status == 0:
    ...
    data.GetXaxis().SetTitle("Minimum #gamma ID MVA")
    ...
    #h_phoId_sba0.Scale(0.7 * 1.88 * 5./100)
    ...
    # Display the canvas
    c.Draw()
    c.SaveAs(outputdir+"/fitRes_minPhoId_1d.png")
    c.SaveAs(outputdir+"/fitRes_minPhoId_1d.pdf")
