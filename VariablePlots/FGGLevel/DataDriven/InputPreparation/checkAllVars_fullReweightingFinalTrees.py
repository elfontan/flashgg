...
sideband = TFile("/eos/user/e/elfontan/DiPhotonAnalysis/diphotonBDT/NTUPLES_Oct2023/sb_newfile_final_fullReweighting_allSbEvents.root","READ") 
dipho = TFile("/eos/user/e/elfontan/DiPhotonAnalysis/diphotonBDT/NTUPLES_Oct2023/mgg0_newfile_final_fullReweighting_allSbEvents.root", "READ")
...
mgg0 = dipho.Get("tagsDumper/trees/mgg_bkg_13TeV_UntaggedTag_0")
...
label_list = ["m_{#gamma#gamma}", "Leading photon p_{T}", "Subleading photon p_{T}", "Leading photon #eta", "Sublead photon #eta", "cosphi", "Lead #gamma IDMVA", "Sublead #gamma IDMVA", "#sigma_{RV}/m_{#gamma#gamma}", "#sigma_{WV}/m_{#gamma#gamma}", "Vtx probability", "Leading #gamma #sigma_{E}/E", "Subleading #gamma #sigma_{E}/E", "Diphoton p_{T}", "Diphoton sum p_{T}"]
...
for variable in var_list:
    ...
    # Fill the histograms
    # -------------------
    #sb0.Draw(variable + ">>h_" + variable + "_sbb0", "CMS_hgg_mass>0 && event%20==0 && min(dipho_leadIDMVA,dipho_subleadIDMVA)>0.7 && max(dipho_leadIDMVA,dipho_subleadIDMVA)>0.7", "goff")
    #sb0.Draw(variable + ">>h_" + variable + "_sba0", "weight*weight_allDD*(CMS_hgg_mass>0 && min(dipho_leadIDMVA,dipho_subleadIDMVA)>0.7 && max(dipho_leadIDMVA,dipho_subleadIDMVA)>0.7)", "goff")
    #dat0.Draw(variable + ">>h_" + variable + "_pre0", "CMS_hgg_mass>0 && event%20==0 && min(dipho_leadIDMVA,dipho_subleadIDMVA)>0.7 && max(dipho_leadIDMVA,dipho_subleadIDMVA)>0.7", "goff")
    #mgg0.Draw(variable + ">>h_" + variable + "_mgg0", "weight*weight_allDD*(CMS_hgg_mass>0 && min(dipho_leadIDMVA,dipho_subleadIDMVA)>0.7 && max(dipho_leadIDMVA,dipho_subleadIDMVA)>0.7)", "goff")

    sb0.Draw(variable + ">>h_" + variable + "_sbb0", "(CMS_hgg_mass>0 && event%20==0)", "goff")                                                             
    sb0.Draw(variable + ">>h_" + variable + "_sba0", "weight*weight_allDD*(CMS_hgg_mass>0)", "goff")                                              
...
for variable in var_list:
    ...
    c1.Update()
    c1.SaveAs(outputdir+"/"+variable+"_fullReweighting.png")
    c1.SaveAs(outputdir+"/"+variable+"_fullReweighting.pdf")
